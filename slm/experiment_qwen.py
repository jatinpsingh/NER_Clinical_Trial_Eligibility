"""Qwen SLM prompted-arm runner for CHIA NER.

Mirrors experiments/experiment.py (same GUIDELINES system prompt, same few-shot
superset, same locate()/resume/output layout so results are directly comparable
to the Gemma arm), differing only in that it disables Qwen's reasoning mode.

Qwen3.x are reasoning models with thinking ON by default: the reasoning goes to a
separate `thinking` field and `message.content` comes back EMPTY -> every row
fails to parse (and it's ~20x slower). Verified useless here: on qwen3.5:0.8b
thinking scored 0.000 F1 vs 0.299 strict with it off. So every request pins
`"think": false` — there is no toggle.
"""
...
def call_ollama(model: str, messages: list[dict], temperature: float, num_ctx: int,
    ...
    resp = requests.post(
        ...
        json={
            ...
            "messages": messages,
            "stream": False,
            # Qwen3.x default to thinking ON, which empties message.content. Pin it
            # off so the model emits the JSON directly (do not remove this line).
            "think": False,
            ...
            # num_predict caps generation: small models occasionally go degenerate under
            # the JSON grammar and would otherwise run to num_ctx (~65k tokens) and blow
            # past the read timeout, aborting the whole run. One criterion's entity JSON
            # is at most a few hundred tokens, so this never truncates a valid response.
...
def predict(row: dict, model: str, examples: list[dict], temperature: float,
            num_ctx: int, seed: int, num_predict: int) -> tuple[dict, list[dict], list[dict], str, bool]:
    """Runs in a worker thread. Only a real connection failure propagates (aborts the
    run so it can be resumed); a per-row read timeout or ANY malformed model output is
    caught and recorded as an empty prediction so one bad row never kills the batch."""
    try:
        raw = call_ollama(model, build_messages(examples, row["text"]), temperature, num_ctx, seed, num_predict)
        ...
        # this one request was too slow (e.g. degenerate generation). Fail just this
        # row and keep going — the run is resumable, so it isn't lost.
        return row, [], [], "FAIL (Timeout)", False
...
def main() -> None:
    ...
    ap.add_argument("--model", default="qwen3.5:4b", help="Ollama model tag (default qwen3.5:4b)")
