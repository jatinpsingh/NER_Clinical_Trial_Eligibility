...
def call_ollama(model: str, messages: list[dict], temperature: float, num_ctx: int,
    ...
    resp = requests.post(
        ...
        json={
            ...
            # num_predict caps generation: under the JSON grammar a model can occasionally
            # go degenerate and would otherwise run to num_ctx (~65k tokens) and blow past
            # the read timeout, aborting the whole run. One criterion's entity JSON is at
            # most a few hundred tokens, so this never truncates a valid response.
