"""Qwen prompted-arm runner on the CHIA TEST split.

Same job as experiment_qwen.py (which runs val), with two differences:
...
locate(), resume, output row layout) is imported from experiment_qwen rather than
...
runners differ solely in which split they read. That import is also what pins
`"think": false` on every request — Qwen3.x default to thinking ON, which empties
message.content and fails every row.
...
Usage:
    python test_qwen.py --model qwen3.5:9b -n 500
    python test_qwen.py --model qwen3.5:9b -n 500 -m 100      # 100-row smoke test
    python test_qwen.py --model qwen3.5:9b -n 500 --score-only  # re-score, no inference
"""
...
from experiment_qwen import check_ollama, predict  # noqa: E402
...
def main() -> None:
    ...
    ap.add_argument("--model", default="qwen3.5:9b", help="Ollama model tag (default qwen3.5:9b)")
